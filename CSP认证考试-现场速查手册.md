...
> **适用条件**：数据**有序**（或能判断"左边全满足、右边全不满足"）。看到"最大值最小"或"最小值最大"就用二分答案。
...
> **适用条件**：DFS 用于"枚举所有可能"（n≤20 为佳），BFS 用于"最短步数"（每条边权值相同）。
...
> **适用条件**：题目能通过"每一步选当前最优"得到全局最优。典型特征：排序后贪心选择、区间问题。
...
> **适用条件**：题目含"最优""最大/最小""方案数"，且能从**子问题推导**。核心公式：`dp[当前] = 最优(dp[之前若干状态])`。

#### 0-1 背包

> **适用条件**："每个物品要么选要么不选，容量有限"。例如：有限预算下的最大收益。
...
> **适用条件**："自顶向下或自底向上走格子，求最大/最小路径和"。这是**DP 入门题**，T3/T4 常考类似结构。
...
> **适用条件**：题目出现"节点""边""路径""连通"等词。n≤2000 用邻接表，n≤500 可用邻接矩阵。
...
> **适用条件**："分组""连通""是否在同一集合""合并集合"。T2/T3 常考。
...
> **适用条件**：题目明显涉及数学运算、质数、GCD/LCM、快速幂取模。
...
> **适用条件**：T1-T3 常考字符串解析、提取数字、大小写转换、子串查找。
