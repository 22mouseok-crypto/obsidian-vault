...
\begin{figure}[H]
\centering
% 【需要截图】阶段1终端输出：8次Insert的日志，显示每次插入的明文、local_table频率统计、以及"正常"状态
\fbox{\parbox{0.85\textwidth}{\vspace{2cm}\centering 此处请插入阶段1的终端输出截图\\（从"阶段1：混合插入多类型数据"到第8条插入完成，第9-16行）}}
\caption{阶段1：混合插入输出}
\label{fig:phase1}
\end{figure}
...
30次插入'x'的过程中，始终没有发生分裂（每次Insert返回False）。原因有三：一是初始编码空间非常大（$[0, 2^{62}]$），30个编码完全不会耗尽区间；二是每次取区间中点，间隔非常充裕；三是30远未达到节点容量128。这也从侧面说明，在正常使用场景下Recode并不会频繁触发。

插入后执行Search('b', 'p')，成功返回了banana、cherry、orange的密文，且解密正确，说明重复插入没有破坏范围查询功能。

\begin{figure}[H]
\centering
% 【需要截图】阶段2终端输出：30次插入'x'的日志（显示频率递增），以及Search('b','p')的范围查询结果（4条解密后的记录）
\fbox{\parbox{0.85\textwidth}{\vspace{3cm}\centering 此处请插入阶段2的终端输出截图\\（30次插入'x' + 范围查询结果）}}
...
\begin{itemize}
    \item 两个"apple"记录的编码值相差巨大（$7.21\times 10^{16}$ vs $2.88\times 10^{17}$），无法从编码推断它们对应同一明文。
    \item 30条"x"记录分布在约$3.06\times 10^{17}$到$3.07\times 10^{18}$的宽阔区间内，编码之间有明显的间隔，频率信息被完全隐藏。
    \item 不同明文的编码在整体上保持了与插入位置一致的顺序。由于CalPos函数根据字典序计算位置（apple < banana < cherry < orange < pear < x），每个新插入的明文都会被分配到正确的区间，因此编码值整体上反映了这一顺序关系。需要注意的是，编码值只保证$\le$关系与B+树中的位置一致，并不一定是严格连续或完全等间距的。另外，从数据中可以看到，同一明文的多个副本（如两个cherry、两个orange）的编码值分别位于'x'编码区的两侧，这是因为它们在阶段1中先于'x'被插入，B+树在为后续的'x'分配编码时，在它们各自所在的叶子节点区间内取了中间值，导致了编码值的穿插。但这不影响保序性——范围查询时通过FHSearch定位的是B+树中的位置，而非直接比较编码值。
\end{itemize}
...
明文 & 出现次数 & 编码范围（近似） \\
\midrule
apple & 2 & $7.21\times 10^{16} \sim 2.88\times 10^{17}$ \\
banana & 1 & $3.06\times 10^{17}$ \\
cherry & 2 & $3.07\times 10^{17} \sim 3.07\times 10^{18}$ \\
orange & 2 & $3.10\times 10^{17} \sim 3.11\times 10^{17}$ \\
x & 30 & $3.07\times 10^{17} \sim 3.07\times 10^{18}$ \\
pear & 1 & $3.60\times 10^{17}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
% 【需要截图】阶段3终端输出：数据库中所有记录的 encoding 和 plaintext 对照表
\fbox{\parbox{0.85\textwidth}{\vspace{3cm}\centering 此处请插入阶段3的编码分布完整输出截图\\（所有记录的 encoding 和对应明文）}}
\caption{阶段3：数据库编码分布}
\label{fig:phase3}
\end{figure}

\subsection{程序异常分析与修复}

在阶段3的编码分布查询末尾，程序抛出了异常：
...
\begin{enumerate}
    \item \textbf{重复调用FHInsert}。client.py的Insert函数中，第61行先执行了一次\lstinline|select FHInsert(...)|来判断是否分裂，紧接着第63行调用\lstinline|call pro_insert(...)|，而存储过程内部又会调用一次FHInsert。这导致同一条密文被插入了B+树两次——B+树的记录数和数据库表的记录数不一致，随着插入增多，两者之间的状态偏差越来越严重，最终在查询编码时出现了无法解密的情况。修复方法是去掉第一次FHInsert调用，改为在存储过程执行后通过FHStart()的返回值判断是否发生了Recode。
...
此外，在解密循环中添加了try-except异常处理，避免单条记录解密失败导致整个程序崩溃，提高了测试代码的健壮性。
