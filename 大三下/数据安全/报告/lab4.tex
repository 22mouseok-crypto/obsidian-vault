...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{datasecuritylab4/混合插入输出.png}
    \caption{阶段1：混合插入输出}
    \label{fig:phase1}
\end{figure}
...
30次插入’x’以及后续20次插入’y’的过程中始终没有发生分裂，总共0次分裂。原因在于初始编码空间非常大（$[0, 2^{62}]$），50多个编码完全不会耗尽区间；每次取区间中点，间隔非常充裕；同时总记录数远低于节点容量128。这也从侧面说明，在正常使用场景下Recode并不会频繁触发。

阶段2结束后执行Search(‘b’, ‘p’)，成功返回了banana、cherry（2条）、orange（2条）共5条记录的密文，且解密正确，说明重复插入没有破坏范围查询功能。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{datasecuritylab4/重复插入’x’及范围查询.png}
    \caption{阶段2：重复插入’x’及范围查询}
    \label{fig:phase2}
\end{figure}

\subsection{阶段3 \& 4：编码分布观察与最终结果}

阶段3查询数据库编码分布后，阶段4继续插入20次’y’，最终编码分布如表\ref{tab:encoding}所示。从结果可以清楚地看到：

\begin{itemize}
    \item 两个"apple"记录的编码值分别为 $2.88\times 10^{17}$ 和 $5.76\times 10^{17}$，相差一倍，攻击者无法从编码推断它们对应同一明文。
    \item 30条"x"记录的编码分布在 $8.83\times 10^{17}$ 到 $1.13\times 10^{18}$ 之间，编码之间呈现明显的等间隔分布（每个间隔约 $1.07\times 10^{16}$），频率信息被完全隐藏。
    \item 不同明文的编码保持了正确的字典序：apple < banana < cherry < orange < pear < x < y，与CalPos按字典序计算位置的逻辑完全一致，保序性得到了保证。
...
由于阶段3和阶段4的终端输出较长（共110余行），截图分为多张。图\ref{fig:phase3}用子图方式将阶段3的编码分布输出拼接展示，图\ref{fig:phase4}展示阶段4插入’y’后的最终数据库编码分布。

\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{datasecuritylab4/大量重复插入1.png}
        \caption{阶段3编码分布（一）}
        \label{fig:phase3a}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{datasecuritylab4/大量重复插入.png}
        \caption{阶段3编码分布（二）}
        \label{fig:phase3b}
    \end{subfigure}
    \caption{阶段3：大量重复插入后的数据库编码分布}
    \label{fig:phase3}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{datasecuritylab4/数据库编码分布.png}
        \caption{最终编码分布（一）}
        \label{fig:phase4a}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{datasecuritylab4/数据库编码分布2.png}
        \caption{最终编码分布（二）}
        \label{fig:phase4b}
    \end{subfigure}
    \vskip\baselineskip
    \begin{subfigure}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{datasecuritylab4/数据库编码分布3.png}
        \caption{最终编码分布（三）}
        \label{fig:phase4c}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{datasecuritylab4/数据库编码分布4.png}
        \caption{最终编码分布（四）}
        \label{fig:phase4d}
    \end{subfigure}
    \caption{阶段4：插入’y’后的最终数据库编码分布}
    \label{fig:phase4}
\end{figure}
...
修复后重新运行，程序正常完成所有4个阶段的测试，不再出现崩溃。
