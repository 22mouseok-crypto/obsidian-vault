...
void Recode(vector<LeafNode *> node_list)
{
    ...
    if ((right_bound - left_bound) > total_cipher_num)
    {
        ...
        // 计算间隔量，使code均匀分布
        long long frag = (right_bound - left_bound) / total_cipher_num;
        assert(frag >= 1);
...
long long LeafNode::Encode(int pos)
{
    ...
    // 1. 修复无符号数溢出隐患：确保 pos 有效且 encoding 不为空时再取前一个值
    if (pos > 0 && !this->encoding.empty())
    {
        left = this->encoding.at(pos - 1);
    }
    
    // 2. 确保当前位置未超出边界时，再读取当前位置的编码作为右边界
    if (!this->encoding.empty() && pos < (int)this->encoding.size())
    {
        right = this->encoding.at(pos);
    }

    // 3. 核心 OPE 区间分配逻辑
    if ((right - left) < 2)
    {
        // 如果该区间已经没有足够的整数空间，则需要 recode
        vector<LeafNode *> node_list;
        node_list.push_back(this);
        Recode(node_list);
        
        // 返回特殊值 0，标志已经调整，需要存储过程去更新数据库
        return 0;
    }
    else
    {
        // 否则直接取 left 和 right 的中间值作为新的 code
        long long cd = left + (right - left) / 2;
        
        // 将新编码插入对应的 pos 位置，保持与密文同步
        this->encoding.insert(this->encoding.begin() + pos, cd);
        return cd;
    }
}
...
void LeafNode::rebalance()
{
    ...
    if (!this->parent)
    {
        // 如果当前结点是root结点，则new一个InternalNode
        InternalNode *new_root = new InternalNode();
        new_root->insert_node(0, this);
        new_root->insert_node(1, new_node);
        root = new_root;
...
// 替换原来的 LeafNode::insert
long long LeafNode::insert(int pos, string cipher)
{
    // 越界保护：如果 pos 大于当前密文集合的大小，强制放到最后
    if (pos < 0) pos = 0;
    if (pos > this->cipher.size()) {
        pos = this->cipher.size();
    }
